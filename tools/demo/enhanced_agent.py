    ...
    if language == "es":
        ...
        if prerequisites:
            ...
            if "tools" in prerequisites:
                print(
                    f"  - Herramientas necesarias: {', '.join(prerequisites['tools'])}"
                )
            if "conditions" in prerequisites:
                print(
                    f"  - Condiciones necesarias: {', '.join(prerequisites['conditions'])}"
                )
            if "permissions" in prerequisites:
                print(
                    f"  - Permisos requeridos: {', '.join(prerequisites['permissions'])}"
                )
        ...
        for input_param in tool["how_to_use"]["inputs"]:
            if input_param["type"] == "object" and "schema" in input_param:
                print(
                    f"    * {input_param['name']} (objeto): {input_param.get('description', 'Sin descripción')}"
                )
                print(f"      Propiedades:")
                for prop_name, prop_details in (
                    input_param["schema"].get("properties", {}).items()
                ):
                    required = (
                        "[Requerido]"
                        if prop_name in input_param["schema"].get("required", [])
                        else ""
                    )
                    prop_desc = prop_details.get("description", "Sin descripción")
                    print(
                        f"        - {prop_name} ({prop_details.get('type', 'indefinido')}): {prop_desc} {required}"
                    )
            else:
                print(
        ...
        if "feedback" in tool:
            ...
            if "progress_indicators" in tool["feedback"]:
                print(
                    f"  - Indicadores de progreso: {', '.join(tool['feedback']['progress_indicators'])}"
                )
            if "completion_signals" in tool["feedback"]:
                print(
                    f"  - Señales de completitud: {', '.join(tool['feedback']['completion_signals'])}"
                )
        ...
        if prerequisites:
            ...
            if "tools" in prerequisites:
                print(
                    f"  - Ferramentas necessárias: {', '.join(prerequisites['tools'])}"
                )
            if "conditions" in prerequisites:
                print(
                    f"  - Condições necessárias: {', '.join(prerequisites['conditions'])}"
                )
            if "permissions" in prerequisites:
                print(
                    f"  - Permissões requeridas: {', '.join(prerequisites['permissions'])}"
                )
        ...
        for input_param in tool["how_to_use"]["inputs"]:
            if input_param["type"] == "object" and "schema" in input_param:
                print(
                    f"    * {input_param['name']} (objeto): {input_param.get('description', 'Sem descrição')}"
                )
                ...
                print(
                    f"    * {input_param['name']} ({input_param['type']}): {input_param.get('description', 'Sem descrição')}"
                )
        ...
        if "feedback" in tool:
            ...
            if "progress_indicators" in tool["feedback"]:
                print(
                    f"  - Indicadores de progresso: {', '.join(tool['feedback']['progress_indicators'])}"
                )
            if "completion_signals" in tool["feedback"]:
                print(
                    f"  - Sinais de conclusão: {', '.join(tool['feedback']['completion_signals'])}"
                )
        ...
        if prerequisites:
            ...
            if "conditions" in prerequisites:
                print(
                    f"  - Required conditions: {', '.join(prerequisites['conditions'])}"
                )
            if "permissions" in prerequisites:
                print(
                    f"  - Required permissions: {', '.join(prerequisites['permissions'])}"
                )
        ...
        for input_param in tool["how_to_use"]["inputs"]:
            if input_param["type"] == "object" and "schema" in input_param:
                print(
                    f"    * {input_param['name']} (object): {input_param.get('description', 'No description')}"
                )
                print(f"      Properties:")
                for prop_name, prop_details in (
                    input_param["schema"].get("properties", {}).items()
                    ...
                    print(
                        f"        - {prop_name} ({prop_details.get('type', 'undefined')}): {prop_desc} {required}"
                    )
            else:
                print(
        ...
        if "feedback" in tool:
            ...
            if "progress_indicators" in tool["feedback"]:
                print(
                    f"  - Progress indicators: {', '.join(tool['feedback']['progress_indicators'])}"
                )
            if "completion_signals" in tool["feedback"]:
                print(
                    f"  - Completion signals: {', '.join(tool['feedback']['completion_signals'])}"
                )
...
def test_goal(tools, goal, language=None, verbose=False):
    ...
    language_name = {"es": "ESPAÑOL", "en": "ENGLISH", "pt": "PORTUGUÊS"}.get(
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["en", "es", "pt"],
        help="Force language for tool selection",
        default=None,
    )
    parser.add_argument(
        "--verbose",
...
def main():
    ...
    test_goal(
        tools, "necesito hacer un agujero preciso en una pared", verbose=args.verbose
    )
    test_goal(tools, "I need to make a precise hole in a wall", verbose=args.verbose)
    test_goal(
        tools, "preciso fazer um furo preciso em uma parede", verbose=args.verbose
    )
