...
class ColorText:
    ...
    @staticmethod
    def bold(text):
        return f"{ColorText.BOLD}{text}{ColorText.RESET}"

    @staticmethod
    def red(text):
        return f"{ColorText.RED}{text}{ColorText.RESET}"

    @staticmethod
    def green(text):
        return f"{ColorText.GREEN}{text}{ColorText.RESET}"

    @staticmethod
    def yellow(text):
        return f"{ColorText.YELLOW}{text}{ColorText.RESET}"

    @staticmethod
    def blue(text):
        return f"{ColorText.BLUE}{text}{ColorText.RESET}"

    @staticmethod
    def magenta(text):
        return f"{ColorText.MAGENTA}{text}{ColorText.RESET}"

    @staticmethod
    def cyan(text):
        return f"{ColorText.CYAN}{text}{ColorText.RESET}"
...
class ATDFShowcase:
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.examples_dir):
            print(
                f"Error: No se encontró el directorio de ejemplos en '{self.examples_dir}'."
            )
            sys.exit(1)
        ...
        print(
            f"Se cargaron {len(self.sdk_toolbox)} herramientas con el SDK y {len(self.tools)} con el cargador mejorado.\n"
        )
    ...
    def print_tool_details(self, tool, language=None):
        ...
        if "localization" in tool_data and language in tool_data["localization"]:
            description = tool_data["localization"][language].get(
                "description", description
            )
            when_to_use = tool_data["localization"][language].get(
        ...
        for input_param in tool_data["how_to_use"]["inputs"]:
            print(
                f"  - {ColorText.magenta(input_param['name'])} ({ColorText.cyan(input_param['type'])}): {input_param.get('description', 'Sin descripción')}"
            )

        # Imprimir salidas
        print(f"\n{ColorText.bold('Salidas:')}")
        print(
            f"  - {ColorText.bold('Éxito:')} {tool_data['how_to_use']['outputs']['success']}"
        )
        ...
        if "examples" in tool_data and tool_data["examples"]:
            ...
            for i, example in enumerate(tool_data["examples"], 1):
                print(
                    f"  {ColorText.bold(f'Ejemplo {i}:')} {example.get('title', 'Sin título')}"
                )
    ...
    def menu_principal(self):
        ...
        while True:
            self.print_header(
                "ATDF Showcase - Demostración de Agent Tool Description Format"
            )
            ...
            print(
                f"  3. {ColorText.yellow('Convertir herramienta básica a formato mejorado')}"
            )
    ...
    def menu_explorar_herramientas(self):
        ...
        while True:
            ...
            for i, tool in enumerate(self.sdk_toolbox, 1):
                print(
                    f"  {i}. {ColorText.green(tool.tool_id)}: {tool.description[:60]}{'...' if len(tool.description) > 60 else ''}"
                )

            print(f"\n{ColorText.cyan('Opciones:')}")
            print(
                f"  [1-{len(self.sdk_toolbox)}] {ColorText.yellow('Ver detalles de una herramienta')}"
            )
    ...
    def menu_probar_agente(self):
        ...
        print(
            f"{ColorText.cyan('Puede hacer consultas en Español, Inglés o Portugués.')}"
        )
        ...
        while True:
            ...
            if choice == "0":
                ...
                if not query.strip():
                    print(
                        ColorText.red(
                            "Consulta vacía. Por favor, ingrese una consulta válida."
                        )
                    )
                    continue
    ...
    def menu_convertir_herramienta(self):
        ...
        for file in os.listdir(self.examples_dir):
            if (
                file.endswith(".json")
                and "enhanced" not in file
                and not file.endswith("_pt.json")
                and not file.endswith("_es.json")
                and not file.endswith("_en.json")
            ):
        ...
        print(f"\n{ColorText.cyan('Opciones:')}")
        print(
            f"  [1-{len(basic_tools)}] {ColorText.yellow('Convertir una herramienta')}"
        )
        print(f"  0. {ColorText.red('Volver al menú principal')}")
        ...
        if choice == "0":
            ...
            enhanced_tool = convert_to_enhanced(
                basic_tool, author=author, extract_language=True
            )
            ...
            if "metadata" in enhanced_tool:
                print(
                    f"{ColorText.green('✓')} {ColorText.bold('Metadatos')}: {len(enhanced_tool['metadata'])} campos"
                )

            if "examples" in enhanced_tool:
                print(
                    f"{ColorText.green('✓')} {ColorText.bold('Ejemplos')}: {len(enhanced_tool['examples'])} ejemplos"
                )

            if "localization" in enhanced_tool:
                langs = list(enhanced_tool["localization"].keys())
                print(
                    f"{ColorText.green('✓')} {ColorText.bold('Localización')}: {len(langs)} idiomas ({', '.join(langs)})"
                )

            if "prerequisites" in enhanced_tool:
                print(
                    f"{ColorText.green('✓')} {ColorText.bold('Prerrequisitos')}: {len(enhanced_tool['prerequisites'])} categorías"
                )

            if "feedback" in enhanced_tool:
                print(
                    f"{ColorText.green('✓')} {ColorText.bold('Feedback')}: {len(enhanced_tool['feedback'])} tipos"
                )
    ...
    def menu_comparar_versiones(self):
        ...
        basic_schema_file = os.path.join(
            os.path.dirname(
                os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            ),
            "schema/atdf_schema.json",
        )
        enhanced_schema_file = os.path.join(
            os.path.dirname(
                os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            ),
        ...
        if not os.path.exists(basic_schema_file) or not os.path.exists(
            enhanced_schema_file
        ):
        ...
        try:
            ...
            print(
                f"{ColorText.bold('Propiedades requeridas en ATDF básico:')} {', '.join(basic_required)}"
            )
            print(
                f"{ColorText.bold('Propiedades requeridas en ATDF mejorado:')} {', '.join(enhanced_required)}"
            )
            ...
            for prop in sorted(new_properties):
                if prop in enhanced_schema.get("properties", {}):
                    desc = enhanced_schema["properties"][prop].get(
