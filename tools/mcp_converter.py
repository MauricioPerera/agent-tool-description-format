...
def mcp_to_atdf(mcp_tool: Dict[str, Any], 
                enhanced: bool = False,
                author: str = "MCP Converter") -> Dict[str, Any]:
    """
...
def _extract_when_to_use(mcp_tool: Dict[str, Any]) -> str:
    ...
    if "annotations" in mcp_tool:
        context_hints = [
            f"Uso: {hint}" for key, hint in mcp_tool["annotations"].items()
    ...
    return f"Usar cuando se requiera {mcp_tool.get('description', '').lower().strip('.')}"[:200]
...
def _default_outputs(success_msg: str = None, 
                     failure_codes: List[Dict[str, str]] = None) -> Dict[str, Any]:
    """
...
def convert_mcp_file(input_file: Union[str, Path], 
                    output_file: Optional[Union[str, Path]] = None,
                    format: str = "json",
                    enhanced: bool = False) -> Optional[Dict[str, Any]]:
    """
