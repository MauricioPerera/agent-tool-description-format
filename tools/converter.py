...
def save_tool(
    tool: Dict[str, Any], file_path: Union[str, Path], format: str = "json"
) -> bool:
    """
...
def convert_to_enhanced(
    basic_tool: Dict[str, Any],
    author: str = "ATDF Converter",
    ...
    if extract_language:
        ...
        es_count = sum(
            ...
            if word in description.lower() or word in when_to_use.lower()
        )
        pt_count = sum(
            1
        ...
        enhanced_tool["localization"] = {
            current_language: {"description": description, "when_to_use": when_to_use}
        }

        # Actualizar en inglés si no lo está
        if current_language != "en":
            enhanced_tool["description"] = (
                f"[Auto-translated from {current_language}] {description}"
            )
            enhanced_tool["when_to_use"] = (
                f"[Auto-translated from {current_language}] {when_to_use}"
            )

    # Añadir sección de prerequisitos
    enhanced_tool["prerequisites"] = {"tools": [], "conditions": [], "permissions": []}
    ...
    enhanced_tool["examples"] = [
        {
            ...
            "input_values": {},
            "expected_result": basic_tool.get("how_to_use", {})
            .get("outputs", {})
            .get("success", "Success"),
        }
    ]
    ...
    logger.info(
        f"Herramienta convertida a formato mejorado: {enhanced_tool.get('tool_id', 'unknown')}"
    )
    return enhanced_tool


def convert_to_basic(
    enhanced_tool: Dict[str, Any], preserve_id_field: bool = True
) -> Dict[str, Any]:
    """
    ...
    basic_tool = {
        ...
        "how_to_use": enhanced_tool.get(
            "how_to_use",
            {"inputs": [], "outputs": {"success": "Success", "failure": []}},
        ),
    }
    ...
    if "how_to_use" not in enhanced_tool:
        basic_tool["how_to_use"] = {
            ...
            "outputs": {
                ...
                "failure": [
                    {"code": "error_default", "description": "An error occurred"}
                ],
            },
        }

    logger.info(
        f"Herramienta convertida a formato básico: {basic_tool.get('tool_id', basic_tool.get('id', 'unknown'))}"
    )
    return basic_tool
