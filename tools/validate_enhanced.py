...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validador de formato ATDF mejorado (v0.2.0)")
    parser.add_argument("--input", "-i", help="Archivo JSON con la descripción ATDF a validar", required=True)
    parser.add_argument("--schema", "-s", help="Archivo JSON con el esquema ATDF mejorado")
    
    args = parser.parse_args()
    
    # Si no se proporciona un esquema, usar el predeterminado
    if not args.schema:
        args.schema = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                               "schema/enhanced_atdf_schema.json")
    
    # Validar la herramienta
    if validate_tool(args.input, args.schema):
        print(f"✅ Validación exitosa: '{args.input}' cumple con el esquema ATDF mejorado.")
        sys.exit(0)
    else:
