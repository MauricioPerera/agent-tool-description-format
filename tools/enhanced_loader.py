            ...
            if tool:
                ...
                if '_es.' in filename:
                    ...
                    if tool['localization']:
                        tool['metadata']['_language'] = next(iter(tool['localization'].keys()))
                else:
...
def detect_language(text):
    ...
    es_markers = ['hacer', 'crear', 'usar', 'cuando', 'necesites', 'agujero', 'traducir', 'texto', 'permite', 'superficie', 'herramienta']
    en_markers = ['make', 'create', 'use', 'when', 'need', 'hole', 'translate', 'text', 'creates', 'surface', 'tool']
    pt_markers = ['criar', 'fazer', 'usar', 'quando', 'precisar', 'furo', 'traduzir', 'texto', 'permite', 'superfície', 'ferramenta']
...
def select_tool_by_goal(tools, goal, user_language=None):
    ...
    def keyword_match(tool, keywords):
        ...
        for keyword in keywords:
            if (keyword in desc or keyword in when or keyword in tags or
                any(keyword in example.get('title', '').lower() 
    ...
    for tool in other_tools:
        if exact_match(tool):
            logger.info(f"Found exact match in non-preferred language: {tool['tool_id']}")
            return tool
    
    # 5. Try keyword match in all tools
    for tool in other_tools:
        if keyword_match(tool, keywords):
            logger.info(f"Found keyword match in non-preferred language: {tool['tool_id']}")
            return tool
    ...
    for tool in tools:
        for example in tool.get('examples', []):
            ...
            if (goal.lower() in example_title or goal.lower() in example_desc or
                any(kw in example_title or kw in example_desc for kw in keywords)):
...
def extract_keywords(text, language='en'):
    ...
    stopwords = {
        'en': ['the', 'a', 'an', 'and', 'or', 'but', 'if', 'because', 'as', 'what', 'when', 
               'where', 'how', 'why', 'which', 'who', 'whom', 'this', 'that', 'these', 'those', 
               'am', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 
               'do', 'does', 'did', 'can', 'could', 'should', 'would', 'i', 'you', 'he', 'she', 
               'we', 'they', 'need', 'want', 'use'],
        'es': ['el', 'la', 'los', 'las', 'un', 'una', 'unos', 'unas', 'y', 'o', 'pero', 'si', 
               'porque', 'como', 'qué', 'cuando', 'donde', 'cómo', 'por qué', 'quién', 'quienes', 
               'este', 'esta', 'estos', 'estas', 'ese', 'esa', 'esos', 'esas', 'soy', 'eres', 
               'es', 'somos', 'son', 'era', 'eras', 'éramos', 'eran', 'ser', 'sido', 'estar', 
               'he', 'has', 'ha', 'hemos', 'han', 'hago', 'haces', 'hace', 'hacemos', 'hacen', 
               'puedo', 'puedes', 'puede', 'podemos', 'pueden', 'yo', 'tú', 'él', 'ella', 
               'nosotros', 'ellos', 'ellas', 'necesito', 'quiero', 'usar'],
        'pt': ['o', 'a', 'os', 'as', 'um', 'uma', 'uns', 'umas', 'e', 'ou', 'mas', 'se', 
               'porque', 'como', 'que', 'quando', 'onde', 'como', 'por que', 'quem', 'quais', 
               'este', 'esta', 'estes', 'estas', 'esse', 'essa', 'esses', 'essas', 'sou', 'és', 
               'é', 'somos', 'são', 'era', 'eras', 'éramos', 'eram', 'ser', 'sido', 'estar', 
               'tenho', 'tens', 'tem', 'temos', 'têm', 'faço', 'fazes', 'faz', 'fazemos', 'fazem', 
               'posso', 'podes', 'pode', 'podemos', 'podem', 'eu', 'tu', 'ele', 'ela', 
               'nós', 'eles', 'elas', 'preciso', 'quero', 'usar']
    }
    ...
    important_phrases = {
        ...
        'es': ['hacer un agujero', 'crear un agujero', 'traducir texto', 'pintar un cuadro'],
...
def get_tool_examples(tool, num_examples=2, language=None):
    ...
    if language and 'localization' in tool and language in tool['localization']:
        ...
        language_examples = [ex for ex in tool['examples'] 
