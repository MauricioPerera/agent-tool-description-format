...
Use POST `/feedback` to registrar el resultado de una ejecuciÃ³n y ajustar el ranking:
...
1. **Semantic ranking** â€“ plug sentence embeddings (e.g. `sentence-transformers`) and LanceDB/pgvector for similarity search beyond keyword heuristics.
2. **Observability** â€“ expose Prometheus metrics (queries served, latency, precision) and integrate with the existing monitoring stack.
3. **Integration tests** â€“ add end-to-end tests that hit `/recommend` while the MCP bridge serves live data and validate n8n HTTP-node integration.
4. **Admin tooling** â€“ add CLI/API commands to remove servers, inspect inactive tools, and trigger selective re-syncs.
