...
class TrilingualTester:
    ...
    def __init__(self):
        ...
        self.examples_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "schema/examples")
    ...
    def run_all_tests(self):
        ...
        if not self.tools:
            logger.error(f"No se pudieron cargar herramientas desde '{self.examples_dir}'.")
            return False
    ...
    def test_spanish_queries(self):
        ...
        for i, query in enumerate(queries, 1):
            ...
            expected_tool_type = "hole" if any(word in query for word in ["agujero", "perforar", "huecos"]) else "translator"
    ...
    def test_english_queries(self):
        ...
        for i, query in enumerate(queries, 1):
            ...
            # Determinar la herramienta esperada
            expected_tool_type = "hole" if any(word in query for word in ["hole", "drill"]) else "translator"
    ...
    def test_portuguese_queries(self):
        ...
        for i, query in enumerate(queries, 1):
            ...
            # Determinar la herramienta esperada
            expected_tool_type = "hole" if any(word in query for word in ["furo", "perfurar", "buraco"]) else "translator"
