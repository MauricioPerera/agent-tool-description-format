...
class EnhancedFeaturesTester:
    ...
    def _cleanup(self):
        ...
        try:
            ...
            logger.info(
                f"Directorio temporal '{self.temp_dir}' eliminado correctamente."
            )
    ...
    def test_conversion_metadata(self):
        ...
        enhanced_tool = convert_to_enhanced(
            basic_tool, author="Test Author", extract_language=True
        )

        # Verificar metadatos
        has_metadata = "metadata" in enhanced_tool
        has_author = (
            has_metadata and enhanced_tool["metadata"].get("author") == "Test Author"
        )
        has_tags = (
            has_metadata
            and "tags" in enhanced_tool["metadata"]
            and len(enhanced_tool["metadata"]["tags"]) > 0
        )
    ...
    def test_conversion_examples(self):
        ...
        # Verificar ejemplos
        has_examples = (
            "examples" in enhanced_tool and len(enhanced_tool["examples"]) > 0
        )
        ...
        if has_examples and len(enhanced_tool["examples"]) > 0:
            ...
            example_structure = (
                "goal" in first_example
                and "input_values" in first_example
                and "expected_result" in first_example
            )
    ...
    def test_conversion_localization(self):
        ...
        if has_localization and has_multiple_langs:
            ...
            localization_structure = (
                "description" in enhanced_tool["localization"][lang]
                and "when_to_use" in enhanced_tool["localization"][lang]
            )
    ...
    def test_conversion_prerequisites(self):
        ...
        if has_prerequisites:
            prerequisites_structure = (
                "tools" in enhanced_tool["prerequisites"]
                and "conditions" in enhanced_tool["prerequisites"]
                and "permissions" in enhanced_tool["prerequisites"]
            )
    ...
    def test_conversion_feedback(self):
        ...
        if has_feedback:
            feedback_structure = (
                "progress_indicators" in enhanced_tool["feedback"]
                and "completion_signals" in enhanced_tool["feedback"]
            )
