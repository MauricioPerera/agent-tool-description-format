...
class ATDFTester:
    ...
    def test_sdk_loading(self):
        ...
        success = specific_toolbox.load_tool_from_file(f"{self.examples_dir}/hole_maker.json")
        self._log_result(
        ...
        if len(toolbox) > 0:
            ...
            properties_exist = (
                hasattr(tool, 'tool_id') and 
                hasattr(tool, 'description') and 
                hasattr(tool, 'when_to_use') and 
                hasattr(tool, 'inputs')
            )
            self._log_result(
        ...
        if os.path.exists(advanced_tool_path):
            with open(advanced_tool_path, 'r', encoding='utf-8') as f:
                ...
                if has_localization and len(langs) > 1:
                    ...
                    for lang in langs:
                        if 'description' not in tool_data['localization'][lang] or 'when_to_use' not in tool_data['localization'][lang]:
