...
async def index_tools(args) -> None:
    ...
    if args.tools_dir:
        ...
        logger.info(f"Se cargaron {len(tools_data)} herramientas desde {args.tools_dir}")
        ...
        try:
            with open(args.tools_file, 'r', encoding='utf-8') as f:
                ...
                if isinstance(file_tools, list):
                    ...
                    logger.info(f"Se cargaron {len(file_tools)} herramientas desde {args.tools_file}")
                else:
                    logger.error(f"El archivo {args.tools_file} no contiene una lista de herramientas")
        except Exception as e:
...
async def search_tools(args) -> None:
    ...
    if not os.path.exists(args.db_path):
        ...
        logger.info("Primero debes indexar herramientas con: python sdk/vector_search_cli.py index")
        return
...
async def manage_db(args) -> None:
    ...
    if args.info:
        ...
        # Mostrar información
        db_size = sum(f.stat().st_size for f in Path(args.db_path).glob('**/*') if f.is_file())
        ...
        if os.path.exists(args.db_path):
            ...
            confirm = input(f"¿Estás seguro de que quieres eliminar la base de datos en {args.db_path}? (s/N): ")
            if confirm.lower() == 's':
                try:
...
def main():
    ...
    # Comando: index
    index_parser = subparsers.add_parser('index', help='Indexar herramientas en la base de datos')
    index_parser.add_argument('--tools-dir', help='Directorio con herramientas ATDF (archivos JSON)')
    index_parser.add_argument('--tools-file', help='Archivo JSON con lista de herramientas ATDF')
    index_parser.add_argument('--db-path', default='./vector_db', help='Ruta para la base de datos vectorial')
    index_parser.add_argument('--model', default='all-MiniLM-L6-v2', help='Modelo de embeddings a utilizar')
    
    # Comando: search
    search_parser = subparsers.add_parser('search', help='Buscar herramientas mediante consulta semántica')
    search_parser.add_argument('query', help='Consulta para buscar herramientas')
    search_parser.add_argument('--limit', type=int, default=5, help='Número máximo de resultados')
    search_parser.add_argument('--lang', help='Código de idioma para la búsqueda (ej: es, en)')
    search_parser.add_argument('--category', help='Filtrar por categoría')
    search_parser.add_argument('--tags', help='Filtrar por etiquetas (separadas por comas)')
    search_parser.add_argument('--db-path', default='./vector_db', help='Ruta de la base de datos vectorial')
    search_parser.add_argument('--model', default='all-MiniLM-L6-v2', help='Modelo de embeddings a utilizar')
    search_parser.add_argument('--detail', action='store_true', help='Mostrar detalles completos de las herramientas')
    
    # Comando: db
    db_parser = subparsers.add_parser('db', help='Administrar la base de datos vectorial')
    db_parser.add_argument('--info', action='store_true', help='Mostrar información sobre la base de datos')
    db_parser.add_argument('--reset', action='store_true', help='Eliminar la base de datos existente')
    db_parser.add_argument('--db-path', default='./vector_db', help='Ruta de la base de datos vectorial')
    db_parser.add_argument('--model', default='all-MiniLM-L6-v2', help='Modelo de embeddings a utilizar')
