...
function extractWhenToUse(mcpTool) {
    ...
    return `Usar cuando se requiera ${description.toLowerCase().replace(/\.$/, "")}`.substring(0, 200);
}
...
function batchConvert(mcpTools, outputDir, options = {}) {
    ...
    for (let i = 0; i < toolsArray.length; i++) {
        ...
        try {
            ...
            console.log(`✅ Herramienta ${i+1}/${toolsArray.length} convertida: ${outputFile}`);
            results.push(atdfTool);
        } catch (error) {
            console.error(`❌ Error al convertir herramienta ${tool.name}: ${error.message}`);
        }
    }
