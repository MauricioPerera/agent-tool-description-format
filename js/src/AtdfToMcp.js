...
function atdfToMcp(atdfTool, options = {}) {
  ...
  if (!atdfTool.how_to_use.inputs) {
    throw new Error(
      `Campo obligatorio 'how_to_use.inputs' no encontrado en ATDF`
    );
  }
...
function batchConvert(atdfTools, outputDir, options = {}) {
  ...
  for (let i = 0; i < atdfTools.length; i++) {
    ...
    try {
      ...
      console.log(
        `✅ Herramienta ${i + 1}/${atdfTools.length} convertida: ${outputFile}`
      );
      results.push(mcpTool);
    } catch (error) {
      console.error(
        `❌ Error al convertir herramienta ${tool.tool_id}: ${error.message}`
      );
    }
  }
