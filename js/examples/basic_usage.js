...
const { loadToolboxFromDirectory, findBestTool, ATDFTool } = require('../src');
...
function ejemploBuscarHerramientas(toolbox) {
  ...
  if (resultsEn.length > 0) {
    ...
    console.log(
      'No se encontraron herramientas que coincidan con la búsqueda en inglés.'
    );
  }
}
        ...
        failure: [
          {
            ...
            description: 'La operación especificada no es válida',
          },
          {
...
function ejemploCompleto(toolbox) {
  ...
  const peticionUsuario =
    'Necesito hacer un agujero en la pared para colgar un cuadro';
  ...
  const herramienta = findBestTool(toolbox, peticionUsuario, {
  ...
  herramienta.inputs.forEach(input => {
    ...
    console.log(
      `  Valor proporcionado: ${JSON.stringify(entradas[input.name])}`
    );
  });
