...
async function demoVectorSearch() {
  ...
  try {
    ...
    console.error(
      'Para usar búsqueda vectorial, instala las dependencias opcionales:'
    );
  ...
  try {
    ...
    console.log(
      `   ℹ️ Esperando indexación de herramientas en la BD vectorial...`
    );
...
async function runSearchTests(toolbox) {
  ...
  for (const query of queries) {
    ...
    try {
      const vectorResults = await toolbox.searchTools(query, {
...
async function runComparisonExample(toolbox) {
  ...
  console.log(
    '   Esta consulta no contiene palabras clave exactas de ninguna herramienta.'
  );
  ...
  if (standardResults.length === 0) {
    console.log(
      '   - La búsqueda por palabras clave no encontró coincidencias'
    );
  } else {
  ...
  try {
    const vectorResults = await toolbox.searchTools(semanticQuery, {
      useVectorSearch: true,
    });
    ...
    if (vectorResults.length === 0) {
      ...
      vectorResults.forEach((tool, i) => {
        ...
        if (i === 0) {
          console.log(
            `      → Esta herramienta fue detectada por similitud semántica`
          );
  ...
  console.log('\n✨ CONCLUSIÓN:');
  console.log(
    ' • La búsqueda vectorial puede encontrar herramientas semánticamente'
  );
  console.log(
    '   relacionadas incluso cuando no hay coincidencia exacta de palabras.'
  );
  console.log(' • Esto es especialmente útil para consultas ambiguas o cuando');
  console.log(
    '   los usuarios no conocen las palabras exactas para describir la herramienta.'
  );
  console.log(
    ' • En contextos multilingües, la búsqueda vectorial puede encontrar'
  );
  console.log(
    '   herramientas relevantes incluso si la consulta está en otro idioma.'
  );
