...
class ValidationPatterns:
        ...
        try:
            ...
            return self.create_error_response(
                ...
                expected={
                    ...
                    "examples": {
                        "valid_range": {
                            "start_date": (self.now + timedelta(days=min_start_offset)).isoformat(),
                            "end_date": (self.now + timedelta(days=min_start_offset + 14)).isoformat()
                        }
                    }
                },
                validation_results=validation_results,
    ...
    def validate_json(self, json_string: str, field_name: str = "json") -> Dict[str, Any]:
        """Valida formato JSON."""
        # Validar campo requerido
        required_error = self.validate_required_field(json_string, field_name, "JSON string")
        if required_error:
            return required_error
