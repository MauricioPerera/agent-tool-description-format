...
class EnrichedResponseValidator:
    ...
    def validate_password_strength(self, password: str) -> Dict[str, Any]:
        ...
        validations = [
            ...
            ValidationResult("At least one uppercase letter (A-Z)", bool(re.search(r'[A-Z]', password))),
            ValidationResult("At least one lowercase letter (a-z)", bool(re.search(r'[a-z]', password))),
            ValidationResult("At least one number (0-9)", bool(re.search(r'\d', password))),
            ValidationResult("At least one special character (!@#$%^&*(),.?\":{}|<>)", bool(re.search(r'[!@#$%^&*(),.?":{}|<>]', password)))
        ]
