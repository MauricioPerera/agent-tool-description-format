...
class MCPATDFBridge:
    ...
    async def get_tools(self) -> List[MCPTool]:
        ...
        if (self.cache_timestamp and 
            (now - self.cache_timestamp).seconds < self.cache_ttl and 
            self.tools_cache):
...
async def handle_health(request: Request) -> Response:
    ...
    health_data = {
        ...
        "cache_timestamp": bridge.cache_timestamp.isoformat() if bridge.cache_timestamp else None,
        "timestamp": datetime.now().isoformat()
    }
...
async def main():
    ...
    parser.add_argument('--atdf-server', default='http://localhost:8000', 
