...
def demo_mcp_tools():
    ...
    for i, tool in enumerate(tools, 1):
        ...
        print(f"     Esquema de entrada: {len(tool['inputSchema']['properties'])} propiedades")
...
def demo_list_endpoints():
    ...
    if "error" in response:
        ...
        for i, reservation in enumerate(reservations, 1):
            print(f"  {i}. {reservation['guest_name']} - {reservation['room_type']} - {reservation['status']}")
    ...
    if "error" in response:
        ...
        for i, booking in enumerate(bookings, 1):
            print(f"  {i}. {booking['passenger_name']} - {booking['departure_city']} → {booking['arrival_city']} - {booking['status']}")
...
def main():
    ...
    if successful == total:
        ...
        print("✅ La integración FastAPI MCP con manejo de errores ATDF está funcionando correctamente.")
    else:
